...
int count_of_Nodes(Node *root)
{
    if (root == NULL)
    {
        return 0; // number of nodes for empty tree
    }
    int leftc = count_of_Nodes(root->left);
    int rightc = count_of_Nodes(root->right);
...
int main()
{
    ...
    cout << "Number of Nodes : " << count_of_Nodes(root) << endl;
}